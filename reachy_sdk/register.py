...
class Register:
    ...
    def __get__(self, instance: "Register", owner: Type[Any]) -> Any:
        """Unwrap, optionally convert the internal value and return it."""
        value = self.unwrapped_value(instance.__dict__[self.label])
        if self.cvt_to_external is not None:
            value = self.cvt_to_external(value)
        return value

    def __set__(self, instance: "Register", value: Any) -> None:
        ...
        instance.__dict__[self.label] = self.wrapped_value(value)
    ...
    def unwrapped_value(self, value: Any) -> Any:
        ...
        if self.internal_class.__name__ == "PIDValue":
            if value.HasField("pid"):
                ...
                return (
                    value.compliance.cw_compliance_margin,
                    value.compliance.ccw_compliance_margin,
                    value.compliance.cw_compliance_slope,
                    value.compliance.ccw_compliance_slope,
                )
            else:
            ...
            if len(value) == 3:
                ...
                return self.internal_class(
                    compliance=ComplianceMarginSlope(
                        cw_compliance_margin=value[0],
                        ccw_compliance_margin=value[1],
                        cw_compliance_slope=value[2],
                        ccw_compliance_slope=value[3],
                    )
                )
            else:
                raise ValueError("Empty PIDValue received!")
...
T = TypeVar("T", bound="MetaRegister")


class MetaRegister(type):
    """MetaClass used to facilitate the registration of the Register in the Joint."""

    def __new__(cls: Type[T], name: str, bases: Tuple[Type[Any], ...], attrs: Dict[str, Any]) -> T:
...
class MetaRegisterABCMeta(MetaRegister, ABCMeta):
    pass
