...
def test_same_robot(reachy_sdk_zeroed: ReachySDK) -> None:
    ...
    time.sleep(0.2)
    assert reachy_sdk_zeroed.r_arm.get_current_state(round_int=3) == [-10, -10, -10, -10, -10, -10, -10]
    assert reachy2.r_arm.get_current_state(round_int=3) == reachy_sdk_zeroed.r_arm.get_current_state(round_int=3)
    ...
    time.sleep(0.2)
    assert reachy2.r_arm.get_current_state(round_int=3) == [0, 0, 0, 0, 0, 0, 0]
    assert reachy2.r_arm.get_current_state(round_int=3) == reachy_sdk_zeroed.r_arm.get_current_state(round_int=3)
    ...
    while not reachy_sdk_zeroed.is_goto_finished(move1_id):
        ...
        assert np.allclose(
            reachy_sdk_zeroed.l_arm.get_current_state(round_int=3), reachy2.l_arm.get_current_state(round_int=3), atol=1
        )
    assert np.allclose(reachy_sdk_zeroed.l_arm.get_current_state(round_int=3), move1_goal, atol=1e-03)
    assert np.allclose(reachy2.l_arm.get_current_state(round_int=3), move1_goal, atol=1e-03)
