...
def test_odometry_pos(reachy_sdk_zeroed: ReachySDK) -> None:
    if reachy_sdk_zeroed.mobile_base is not None:
        ...
        reachy_sdk_zeroed.mobile_base.set_goal_speed(x=0.5, y=0.0, theta=0)
        tic = time.time()
        ...
        assert np.isclose(odom["theta"], 0.0, atol=1e-02)
        ...
        reachy_sdk_zeroed.mobile_base.set_goal_speed(x=0.0, y=-0.5, theta=0)
        tic = time.time()
        while time.time() - tic < 2:
            reachy_sdk_zeroed.mobile_base.send_speed_command()
            time.sleep(0.01)
        ...
        reachy_sdk_zeroed.mobile_base.set_goal_speed(x=0.0, y=0.0, theta=50)
        tic = time.time()
        while time.time() - tic < 2:
            reachy_sdk_zeroed.mobile_base.send_speed_command()
            time.sleep(0.01)
...
def test_odometry_vel(reachy_sdk_zeroed: ReachySDK) -> None:
    if reachy_sdk_zeroed.mobile_base is not None:
        ...
        def send_y_theta_speeds():
            reachy_sdk_zeroed.mobile_base.set_goal_speed(x=0.0, y=0.7, theta=20)
            tic = time.time()
            while time.time() - tic < 5:
                reachy_sdk_zeroed.mobile_base.send_speed_command()
                time.sleep(0.01)
